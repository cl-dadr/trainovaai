...
import { Camera, Crosshair, Activity, Volume2, XCircle, Zap, Trophy, RotateCcw } from "lucide-react";
...
const CameraPage = () => {
  ...
  const onResults = useCallback((results: any) => {
    ...
    if (results.poseLandmarks) {
      ...
      if (result.repCompleted) {
        ...
        // Show Gen-Z feedback every 5 reps
        if (repFeedbackCounter.current % 5 === 0) {
          showWorkoutFeedback();
        }
      }
    }
  ...
  const startDetection = useCallback(() => {
    ...
    // Show streak notification on start
  ...
  const resetWorkout = () => {
    ...
    setFormScore(0);
    setPlankTime(0);
    setCurrentExercise("unknown");
    setFeedback("Position yourself in frame");
    resetDetection();
    repFeedbackCounter.current = 0;
  };
  ...
  return (
    <div className="relative min-h-screen pb-24 px-4 pt-6">
      ...
      >
        ...
        {isDetecting && (
          <div className="flex items-center gap-2">
            <button onClick={resetWorkout} className="glass-card p-2 rounded-lg">
              <RotateCcw className="h-4 w-4 text-neon-cyan" />
            </button>
            <button onClick={stopDetection} className="glass-card p-2 rounded-lg">
      ...
      >
        ...
        {/* Center prompt when not detecting */}
        {!isDetecting && (
        ...
        <div className="absolute bottom-4 left-4 right-4 flex gap-3 z-10">
          ...
          <div className="glass-card flex-1 p-3 text-center">
            <p className={`text-2xl font-display font-bold ${
              formScore >= 85 ? "text-neon-green" : formScore >= 60 ? "text-neon-orange" : "text-destructive"
            }`}>
      ...
      <motion.div
        ...
        className={`relative z-10 glass-card p-4 flex items-center gap-3 mb-4 border-l-2 ${
          formScore >= 85 ? "border-neon-green/40" : formScore >= 60 ? "border-neon-orange/40" : "border-destructive/40"
        }`}
      >
      ...
      {isDetecting && (
        ...
        >
          <p className="text-xs text-muted-foreground mb-2">
            <Zap className="h-3 w-3 text-neon-green inline mr-1" />
            AI detects: Pushups, Squats, Planks, Lunges, Jumping Jacks, Sit-ups
          </p>
      ...
      {!isDetecting && totalReps > 0 && (
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          className="relative z-10 glass-card p-5 mt-4"
        >
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="text-center">
              <p className="text-2xl font-bold text-neon-green">{totalReps}</p>
              <p className="text-[10px] text-muted-foreground">Total Reps</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-neon-cyan">{formScore}%</p>
              <p className="text-[10px] text-muted-foreground">Avg Form</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-neon-orange">{reps}</p>
              <p className="text-[10px] text-muted-foreground">Last Set</p>
            </div>
          </div>
        </motion.div>
      )}
